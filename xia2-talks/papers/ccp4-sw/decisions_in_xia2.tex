...
also giving guidelines which can be applied in interactive processing.
\end{synopsis}
...
interactively using for example iMosflm it is
...
used, the latter with a ``standard'' scaling model\footnote{Smoothed
  ...
  intervals.}
(the default in CCP4i) and was therefore considered
...
the lattice corresponding to this pointgroup. This is to handle, for
example, a monoclinic lattice with $\beta$ angle indistinguishable
from $90^{\circ}$. 
...
familiar to macromolecular crystallographers deal with static
...
Lactobacillus salivarius UC188 transketolase (E.C. 2.2.1.1, Tkt), a
...
with the cofactor, thiamine pyrophosphate (TPP) and a divalent Mg2+
...
based on the systematic anbsence analysis. After the first round of
scaling the resolution was estimated to be $\sim 2.3$\AA, after which
...
respectively) in refinement. The investigation included a full repeat
...
above. Analysis of this structure however showed that the conclusions
reached by \emph{xia2} were appropriate.
...
The author would like to thank the JCSG, Division of Structural Biology at
the Wellcome Trust Centre for Human Genetics,
SPINE and numerous users for providing the test data which was used in 
developing this software. The author would also like to thank Harry Powell
