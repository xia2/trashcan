...
macromolecular crystallography (MX) is considered as three phases:
...
be subsequently tested. The system must also embody the workflow being
performed to give a structure into which the expertise to
make the choices is embedded. 

\subsection{Strategy}

For any given choice there may be a set of optimum parameters for a
specific case. There may however also be a set of parameters which can be
shown to generally work well with limited knowledge of that specific case. 
...
errors in the direct beam position than the modified DPS procedure in
