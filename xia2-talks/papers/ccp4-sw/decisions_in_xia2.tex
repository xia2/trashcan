...
default parameters which can be 
...
\item{A lattice / cell proposal, based on an analysis of all of the possible
    options.}
\end{itemize}
...
brevity) giving a wide coverage of 
...
\cite{Incardona:wa5014}. With this in mind, the use of few images may
be desirable.
...
\caption{Mean normalised metric penalty as a function of number of
  ...
  a factor of around 1.7, is much larger than any subsequent change.
...
autoindexing with a single image gave the ``correct'' result and
at least $90^{\circ}$ of data were available. The resulting sweeps had 
resolution limits in the range 3.5\AA\ to 1.2\AA\.
First, the number of images
...
\item{all of the metric penalties for a
  given sweep were scaled over the range $0 - 1$}
\item{then this normalized
  value averaged across all sweeps for a given number of images.}
\end{itemize}
...
challenged in subsequent analysis steps.

\subsection{XDS}

Unlike Mosflm and Labelit which assume the $\phi$ centroid is the
centre of the image, 
...
the analysis of intensities with Pointless. If
...
and merged with Scala or Aimless, as this gives an
