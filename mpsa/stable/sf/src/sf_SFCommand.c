...
)
{
  ...
  if((strcmp(argv[1], "Stochastic") == 0) ||
    ...
    sf_BimodalSetup(interp, index + 1, lower, inter, higher);
  ...
  } else if((strcmp(argv[1], "Prop.Bimodal.Form") == 0) ||
	    (strcmp(argv[1], "prop.bimodal.form") == 0)) {
    mpsa_ParticleDefn *Type1;
    mpsa_ParticleDefn *Type2;
    mpsa_Simulation *Sim;
    float m0, t0, tindex, enhance, snval;
    mpsa_List *List;
    mpsa_Link *Link;
    mpsa_Pip *CloudPip;
    ...
    if(argc != 9) {
      Tcl_AppendResult(interp, argv[1], " requires a list, m0, t0, tindex", 
	" lowmasstype and highmasstype, and sn enhancement", (char *) NULL);
      return TCL_ERROR;
    }
    ...
    if(mpsa_GetFloat(interp, argv[8], &enhance) != MPSA_OKAY) {
      return TCL_ERROR;
    } 
	...
	return TCL_ERROR;
      ...
      snval = 1 + enhance * ((cloud_Pip *) Link->Pcl->Pip)->metal;

      if((gwrand48() < (snval * Link->Pcl->mass / m0)) &&
	 (gwrand48() < (snval * pow(((cloud_Pip *) Link->Pcl->Pip)->T / t0, 
           tindex)))) {
	if(sf_BimodalStarForm(Link->Pcl, Type1, Type2, List->Simulation) != 
	  SF_OKAY) {
	  Tcl_AppendResult(interp, "Error forming stars", (char *) NULL);
