 ...
 *
 * 15th january 2001 modified sn shock strength to fit more detailed
 * analytical model, resulting in greater shock strength than KMB
 *
 */
...
static float SNSqrtEfficiency = 1.0;
static float SNSmoothRad = 5.7e-4;
...
{
  SNSqrtEfficiency = sqrt(eff);
  return SN_OKAY;
}

int sn_SetSmoothRadius(
  float rad
)
{
  SNSmoothRad = rad;
...
int sn_CloudInteractionFindKMB(
  float rmin,
  float rmax,
  mpsa_Particle *SNPcl,
  tree_Node *Root
)
{
  ...
  if(dxsq > ((rmax + Root->size) * (rmax + Root->size))) {
    ...
    if(tree_IsNodeOpen(Root) == TREE_OKAY) {
      for(i = 0; i < 8; i++) {
	 sn_CloudInteractionFindKMB(rmin, rmax, SNPcl, &(Root->Branch[i]));
      } 
    } else {
      if((Cloud = Root->Leaf) != NULL) {
	dxsq = 0;
...
)
{
  ...
  /* description of the mark III sn shock model
     use analytical model, where strength (ie E0) is proportional to
     mass of progenitor. v(r) = k * m^0.2 * r^-1.5

     r is smoothed over a distance of 2pc, see shu volume II
     it is assumed here that a 20 solar mass OB will produce a 
     canonical 10^44 J SN

     for Mach no purposes, speed of sound in inter cloud medium about 
     90 km/s ie 0.34 in system units

     efficiency should be about 1e-6

  */

  dx = sqrt(dx + SNSmoothRad);
  dv = sqrt(dv);

  dv += 4.044e-4 * sqrt(SNPcl->mass / 3.57e-10) * 
    pow(dx, -1.5);
  ...
  if(Mach > 10) {
    /* is this the correct treatment of a highly shocked cloud? */
    /* very unlikely - this is like 900 kms-1 */
    /* and accelerate it */
    for(i = 0; i < 3; i++) {
      Cloud->v[i] += SNSqrtEfficiency * 0.75 * (Cloud->x[i] - SNPcl->x[i]) 
        * dv / dx;
    }
  ...
  ((cloud_Pip *) Cloud->Pip)->metal = Mach;
