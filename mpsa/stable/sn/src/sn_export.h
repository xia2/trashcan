...
/*[ sn_CloudInteractionFindMKB
 *[ action:  find clouds to be keith shocked in a tree of clouds
 *[ objects: takes two radii, a sn particle and a tree of clouds
 *[          which have their cloud pips selected
 */

extern int sn_CloudInteractionFindKMB(
  float rmin,
  float rmax,
  mpsa_Particle *SNPcl,
  tree_Node *Root
...
/*[ sn_ShockCloudKMB
 *[ action:  keith shock/cloud interaction physics
 *[ objects: takes a sn particle and a cloud particle
 */

extern int sn_ShockCloudKMB(
  mpsa_Particle *SNPcl,
  mpsa_Particle *Cloud
);
...
extern int sn_SetEff(
  float eff
);

extern int sn_SetSmoothRadius(
  float rad
);
