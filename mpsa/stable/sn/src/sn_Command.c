...
)
{
  ...
  if((strcmp(argv[1], "StartSN") == 0) ||
  ...
  } else if((strcmp(argv[1], "KMBShock") == 0) ||
	    (strcmp(argv[1], "kmbshock") == 0)) {
    tree_Node *Tree;
    mpsa_Link *Link;
    mpsa_List *List;
    mpsa_Pip *SNPip;
	...
	return TCL_ERROR;
      ...
      rmax = ((sn_Pip *) Link->Pcl->Pip)->radius + 
        dt * ((sn_Pip *) Link->Pcl->Pip)->speed;
      sn_CloudInteractionFindKMB(rmin, rmax, Link->Pcl, Tree);
    }
  ...
  } else if((strcmp(argv[1], "SetEff") == 0) ||
	    (strcmp(argv[1], "seteff") == 0)) {
    float NewEff;

    if(argc != 3) {
      Tcl_AppendResult(interp, argv[1], " requires a new efficiency", 
        (char *) NULL);
      return TCL_ERROR;
    }

    if(mpsa_GetFloat(interp, argv[2], &NewEff) != MPSA_OKAY) {
      return TCL_ERROR;
    }
  ...
  } else if((strcmp(argv[1], "SetSmoothRadius") == 0) ||
	    (strcmp(argv[1], "setsmoothradius") == 0)) {
    float NewRad;

    if(argc != 3) {
      Tcl_AppendResult(interp, argv[1], " requires a new radius", 
        (char *) NULL);
      return TCL_ERROR;
    }

    if(mpsa_GetFloat(interp, argv[2], &NewRad) != MPSA_OKAY) {
      return TCL_ERROR;
    }

    sn_SetSmoothRadius(NewRad);
