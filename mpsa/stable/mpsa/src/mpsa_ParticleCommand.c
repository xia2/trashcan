...
)
{
  ...
  if((strcmp(argv[1], "Create") == 0) || 
    ...
    */
  ...
  } else if((strcmp(argv[1], "MeanZ") == 0) |
	    (strcmp(argv[1], "meanz") == 0)) {
    mpsa_List *List;
    ...
    mpsa_ZPosStDev(interp, List);

    return TCL_OK;

  } else if((strcmp(argv[1], "VDisp") == 0) ||
	    (strcmp(argv[1], "vdisp") == 0)) {
    mpsa_Link *Link;
    float stdev, sumvz = 0;
    float sumvzsq = 0;
    mpsa_List *List;
    ...
    for(Link = List->firstLink; Link != NULL; Link = Link->nextLink) {
      sumvz += Link->Pcl->v[2];
      sumvzsq += Link->Pcl->v[2] * Link->Pcl->v[2];
    }

    sumvz /= List->NElements;
    stdev = sqrt((sumvzsq / List->NElements) - sumvz * sumvz);

    sprintf(result, "%e %e", sumvz, stdev);
