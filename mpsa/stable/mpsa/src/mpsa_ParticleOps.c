...
)
{
  ...
  float Range, LogRange, k;
  int i, nbins;
  mpsa_Link *Link;
  float power, meanx, meany, sumxx, sumxy;
  ...
  for(Link = List->firstLink; Link != NULL; Link = Link->nextLink) {
    i = (log10(Link->Pcl->mass/Mlow) / (0.01 * LogRange));
    Bin[i] += Link->Pcl->mass;
  }
  ...
  for(i = 1; i < 100; i++) {
    if(Bin[i] != 0) {
      sumxx += (LogMass[i] - meanx) * (LogMass[i] - meanx);
      sumxy += (LogMass[i] - meanx) * (LogBin[i] - meany);
    }
  }
...
)
{
  ...
  for(Link = List->firstLink; Link != NULL; Link = Link->nextLink) {
    meanz += Link->Pcl->x[2] * Link->Pcl->mass;
    meanzsq += Link->Pcl->x[2] * Link->Pcl->x[2] * Link->Pcl->mass;
    total += Link->Pcl->mass;
  }

  meanz = meanz / total;

  stdev = sqrt((meanzsq / total) - (meanz * meanz));

  sprintf(result, "%f %f", meanz, stdev);
