 ...
 * 
 * modified 4th january 2001: cloud fragmentation remains now move away
 * from original cloud
 *
 */
...
/* notes:
   ...
   5th january 2001
   cloud collision energy dissipation taken in terms of the zero
   momentum frame
...
)
{
  ...
  if(((T1/T2) > 0.1) && ((T1/T2) < 10)) {
    ...
    for(i = 0; i < 3; i++) {
      mva += (PclA->v[i] - newv[i]) * (PclA->v[i] - newv[i]);
      mvb += (PclB->v[i] - newv[i]) * (PclB->v[i] - newv[i]);
    }
...
)
{
  ...
  if(((T1/T2) > 0.1) && ((T1/T2) < 10)) {
    ...
    if(sqrt(v) > (sqrt(NewT / 10) * 3.43e-4)) {
      /* incoming velocity greater than sound speed */
      /* recall that the mach number of a shock is for the moment 
	 being stored in "metal" */
      PipA->metal = v / (sqrt(NewT / 10) * 3.43e-4);
    }
...
/*[ cloud_InteractClouds
 *[ action:  perform the task of interacting two clouds
 *[ objects: takes two cloud particles
 *[ notes:   here is the place to put the cloud cloud collision physics
 */
...
int cloud_InteractClouds(
  mpsa_Particle *PclA,
  mpsa_Particle *PclB,
  float MomTransfer
)
{
  /* purpose of this operation is to slow down the particles
     relative to one another. bit sph like */

  int i;
  float comv[3];

  for(i = 0; i < 3; i++) {
    comv[i] = 0.5 * (PclA->mass * PclA->v[i] + PclB->mass * PclB->v[i]);
  }

  for(i = 0; i < 3; i++) {
    PclA->v[i] = (1 - MomTransfer) * PclA->mass * PclA->v[i] + 
      comv[i] * MomTransfer;
    PclB->v[i] = (1 - MomTransfer) * PclB->mass * PclB->v[i] + 
      comv[i] * MomTransfer;
  }
 ...
 *[ modified 9th january 2001 to give a gaussian distribution to 
 *[          fragmenting clouds
 */
...
)
{
  ...
  while(CloudPcl->mass > CloudMinimumMass) {
    ...
    if(NewCloudMass > CloudPcl->mass) {
      ...
      for(i = 0; i < 3; i++) {
	randomv[i] = gwgauss48();
	dx[i] = 4 * randomv[i] * ((cloud_Pip *) CloudPcl->Pip)->radius
          + CloudPcl->x[i];
	/* v. dispersion from kv */
	dv[i] = randomv[i] * 0.03 + CloudPcl->v[i];
      }
...
/*[ cloud_FragmentCloudDisp
 *[ action:  fragment cloud into many smaller clouds with a given v disp
 *[ objects: takes a cloud particle and a pointer to a simulation
 *[ modified 9th january 2001 to give a gaussian distribution to 
 *[          fragmenting clouds
 */

int cloud_FragmentCloudDisp(
  mpsa_Particle *CloudPcl,
  mpsa_Simulation *Sim,
  mpsa_Pip *CloudPip,
  float vdisp
)
{
  ...
  while(CloudPcl->mass > CloudMinimumMass) {
	...
	dx[i] = 4 * randomv[i] * ((cloud_Pip *) CloudPcl->Pip)->radius
          + CloudPcl->x[i];
	/* v. dispersion from kv */
	dv[i] = randomv[i] * vdisp + CloudPcl->v[i];
      }
      if(mpsa_PclCreateExact(Sim, CloudDefn, NewCloudMass, dx, dv) != 
        MPSA_OKAY) {
	return CLOUD_FAIL;
...
/*[ cloud_TppCollisionSearch2
 *[ action:  searches tpp for clouds to merge and merges them
 *[ objects: takes a cloud particle and a tpp, and a search radius
 */

int cloud_TppCollisionSearch2(
  float Radius,
  float dt,
  mpsa_Particle *Pcl,
  tpp_Node *Node,
...
)
{
  ...
  if(Node->Branch != NULL) {
    ...
    for(i = 0; i < 3; i++) {
      dx = (Pcl->x[i] - Node->centre_x[i]);
      dv = (Pcl->v[i] - Node->centre_v[i]);
      dxsq += dx * dx;
      dxdv += dx * dv;
      dvsq += dv * dv;
    }
    ...
    */
    ...
    /* critical radius, determining where things become irrelevent */

    rmax = 0.8661 * Node->size_x + 0.8661 * dt * Node->size_v;

    if((dxsq + 2 * t * dxdv + t * t * dvsq) < ((2 * Radius + rmax) * 
      (2 * Radius + rmax))) {
      /* node is worth considering */
      for(i = 0; i < 64; i++) {
	cloud_TppCollisionSearch2(Radius, dt, Pcl, &(Node->Branch[i]), List);
      }
    } else {
      /* ignore node - everything in it will miss the target */
    }
  } else {
    Leaf = Node->Leaf;
    if((Leaf != NULL) && (Leaf->type == Pcl->type) && (Leaf != Pcl) &&
       (Leaf->flag != CLOUD_FAIL)) {
      for(i = 0; i < 3; i++) {
	dx = (Pcl->x[i] - Leaf->x[i]);
	dv = (Pcl->v[i] - Leaf->v[i]);
	dxsq += dx * dx;
	...
	t = dt;
      ...
      if((dxsq + 2 * t * dxdv + t * t * dvsq) < ((r1 + r2) * (r1 + r2))) {
	/* particles have collided */
	pairlist_AppendLink(List, Pcl, Leaf, tnew);
      }
    }
  }
...
int cloud_DirectCollisionSearch(
  mpsa_List *List,
...
)
{
  ...
  mpsa_Link *LinkB;
  float t, rA, rB, tnew;
  float dxdv, dxsq, dvsq;
  int i;

  for(LinkA = List->firstLink; LinkA != NULL; LinkA = LinkA->nextLink) {
    for(LinkB = LinkA; LinkB != NULL; LinkB = LinkB->nextLink) {
      if(LinkA->Pcl != LinkB->Pcl) {
	dvsq = 0;
	dxdv = 0;
	dxsq = 0;
	for(i = 0; i < 3; i++) {
	  dxsq += (LinkA->Pcl->x[i] - LinkB->Pcl->x[i]) * 
            (LinkA->Pcl->x[i] - LinkB->Pcl->x[i]);
	  dxdv += (LinkA->Pcl->x[i] - LinkB->Pcl->x[i]) * 
            (LinkA->Pcl->v[i] - LinkB->Pcl->v[i]);
	  dvsq += (LinkA->Pcl->v[i] - LinkB->Pcl->v[i]) * 
            (LinkA->Pcl->v[i] - LinkB->Pcl->v[i]);
	}
	...
	rA = ((cloud_Pip *) LinkA->Pcl->Pip)->radius;
	rB = ((cloud_Pip *) LinkB->Pcl->Pip)->radius;

	if((dxsq + 2 * t * dxdv + t * t * dvsq) < ((rA + rB) * (rA + rB))) {
	  pairlist_AppendLink(PairList, LinkA->Pcl, LinkB->Pcl, tnew);
	}
      }
    }
  }
...
)
{
  ...
  if(dxsq > ((Radius + Node->size) * (Radius + Node->size))) {
    ...
    if(tree_IsNodeOpen(Node) == TREE_OKAY) {
      ...
      for(i = 0; i < 8; i++) {
	cloud_TreeCollisionSearch(Radius, dt, Pcl, &(Node->Branch[i]), List);
      } 
...
int cloud_GridCollisionSearch(
  celllist_List *Grid,
  pairlist_List *List,
  float dt
)
{
  int i, j, k, pclindex, otherindex;
  int x, y, z, counter;
  mpsa_Particle *TestPcl, *OtherPcl;
  float radiusA, radiusB, dxsq, dxdv, dvsq;
  float t, tnew;

  for(i = 0; i < Grid->Size[0]; i++) {
    for(j = 0; j < Grid->Size[1]; j++) {
      for(k = 0; k < Grid->Size[2]; k++) {
	for(pclindex = 0; pclindex < Grid->Grid[i][j][k].NinCell; pclindex++) {
	  TestPcl = Grid->Grid[i][j][k].List[pclindex];
	  radiusA = ((cloud_Pip *) TestPcl->Pip)->radius;
	  for(x = 0; x < 1; x++) {
	    for(y = 0; y < 1; y++) {
	      for(z = 0; z < 1; z++) {
		if(((x + i) < Grid->Size[0]) && 
		   ((y + j) < Grid->Size[1]) &&
		   ((z + k) < Grid->Size[2])) {
		  for(otherindex = 0; otherindex < 
                    Grid->Grid[i+x][j+y][k+z].NinCell; otherindex++) {
		    OtherPcl = Grid->Grid[i+x][j+y][k+z].List[otherindex];
		    if(OtherPcl != TestPcl) {
		      radiusB = ((cloud_Pip *) OtherPcl->Pip)->radius;
		      dxsq = 0;
		      dxdv = 0;
		      dvsq = 0;
		      for(counter = 0; counter < 3; counter++) {
			dxsq += (TestPcl->x[counter] - OtherPcl->x[counter]) * 
			  (TestPcl->x[counter] - OtherPcl->x[counter]);
			dxdv += (TestPcl->x[counter] - OtherPcl->x[counter]) * 
			  (TestPcl->v[counter] - OtherPcl->v[counter]);
			dvsq += (TestPcl->v[counter] - OtherPcl->v[counter]) * 
			  (TestPcl->v[counter] - OtherPcl->v[counter]);
		      }
			...
			t = dt;
		      ...
		      if((dxsq + 2 * t * dxdv + t * t * dvsq) < 
			((radiusA + radiusB) * (radiusA + radiusB))) {
			pairlist_AppendLink(List, TestPcl, OtherPcl, tnew);
		      }
		    }
		  }
		}
...
/*[ cloud_TppCollisionSearchPlus
 *[ action:  searches tpp for clouds to merge and merges them
 *[ objects: takes a cloud particle and a tpp, and a search radius
 */

int cloud_TppCollisionSearchPlus(
  float Radius,
  float dt,
  mpsa_Particle *Pcl,
  tpp_Node *Node,
  pairlist_List *List,
  pairlist_List *MissList
)
{
  mpsa_Particle *Leaf;
  float dxsq = 0, t, v = 0;
  ...
  if(Node->Branch != NULL) {
    ...
    */
    ...
    if((dxsq + 2 * t * dxdv + t * t * dvsq) < ((2 * Radius + rmax) * 
      ...
      /* node is worth considering */
      for(i = 0; i < 64; i++) {
	cloud_TppCollisionSearchPlus(Radius, dt, Pcl, &(Node->Branch[i]), List,
	  MissList);
      }
    } else {
      /* ignore node - everything in it will miss the target */
	...
	/* particles have collided */
	pairlist_AppendLink(List, Pcl, Leaf, tnew);
      } else if((dxsq + 2 * t * dxdv + t * t * dvsq) < (Radius * Radius)) {
	/* particles came close enough to interact */
	pairlist_AppendLink(MissList,  Pcl, Leaf, tnew);
      }
    }
  }
